...
class Task extends Base{
    ...
    public function accept(){
        ...
        $lastTask = \app\home\model\Task::getKeys($this->member['uid'], $item['keywords']);
        if (count($lastTask)>0) {
            message('此任务类型已做过,不能重复抢单','','error');
        }
...
//             }
            ...
            //if($params['give_credit1']>0 || $params['amount']>0){
               ...
               // $status1 = Member::updateCreditById($member['uid'], -$params['give_credit1'], -$params['amount']);
                $status1 = Member::updateCreditById($member['uid'], -$params['amount']);
                if(!$status1){
                    Db::rollback();
...
//         }
        ...
        if (empty($task_operate_steps_contents)) {
            message('您未写操作说明111!','','error');
        }
        foreach ($task_operate_steps_contents as $key => $value) {
            if (empty(trim($value))) {
                message('您未写操作说明222!','','error');
            }
        }
...
//             if($item['give_credit1']>0 || $item['amount']>0){
        if(bccomp($item['amount'], $params['amount'], 2) != 0){
        	if($item['amount']>0){
                $status1 = Member::updateCreditById($member['uid'],$item['amount']);
                if(!$status1){
                    Db::rollback();
