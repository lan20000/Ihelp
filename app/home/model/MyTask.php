...
class MyTask extends Base{
    ...
    public static function outStop($id){
    	return ;
    }
     ...
     * @return unknown
     */
    public static function task_count($uid,$id){
    	$count= self::query("SELECT * FROM tb_task LEFT JOIN tb_task_join ON tb_task.id=tb_task_join.task_id WHERE tb_task.uid=$uid AND tb_task.id=$id AND tb_task_join.`status`=2");
    	return $count;
    }
}