...
class Task extends Base{
    ...
    public static function getListByParams($params = [], $psize = 15){
        //try{
            ...
            $where = "is_complete=0 AND is_display=1 AND outStop=0";
            ...
            if(!empty($tasks)){
                foreach ($tasks as &$v){
                    ...
                    $v['top_end_time']=date('Y-m-d H:i',$v['top_end_time']);
                    $v['take_time']=date('Y-m-d H:i');
                }
            }
            return $tasks;
        /*}catch (Exception $e){
    ...
    //根据ID查询是否有相关关键字任务
    public static function getKeys($id,$keywords){
       return  self::query("SELECT * FROM tb_task LEFT JOIN tb_task_join ON tb_task.id=tb_task_join.task_id WHERE tb_task_join.uid=$id AND tb_task.keywords='$keywords'");
    }
}